...
class ThreeDEPN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv3d(32,16,4,stride=2,padding=1)
        self.conv2 = nn.Conv3d(16,8,4,stride=2,padding=1)
        self.conv3 = nn.Conv3d(8,4,4,stride=2,padding=1)
        self.conv4 = nn.Conv3d(4,2,4,stride=1) # Last conv has no padding and stride=1

        self.bn1 = nn.BatchNorm3d(8) # First layer doesn't get batch norm
        ...
        self.tconv1 = nn.ConvTranspose3d(self.num_features*8*2,self.num_features*8,4,stride=2,padding=1)
        self.tconv2 = nn.ConvTranspose3d(self.num_features*8,self.num_features*4,4,stride=2,padding=1)
        self.tconv3 = nn.ConvTranspose3d(self.num_features*4,self.num_features*2,4,stride=2,padding=1)
        self.tconv4 = nn.ConvTranspose3d(self.num_features*2,self.num_features,4,stride=1)

        self.bn4 = nn.BatchNorm3d(self.num_features*8)
        self.bn5 = nn.BatchNorm3d(self.num_features*4)
        self.bn6 = nn.BatchNorm3d(self.num_features*2) # No batch norm after last layer. Nor ReLU
