        ...
        for i,batch in enumerate(trainloader):
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n']-1):
                ...
                for val_batch in valloader:
                    ...
                    with torch.no_grad():
                        prediction = model(val_batch['points'])

                    _,predicted_labels = torch.max(prediction, dim=1)

                    total += val_batch['label'].shape[0]
                    correct += (predicted_labels==val_batch['label']).sum().item()
                    
                    loss = loss_criterion(prediction,val_batch['label'])
